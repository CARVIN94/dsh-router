...
第二个入口是**设置 → 插件 → dsh-router-core 的详情页**,那里有一节「路由组件」,
把 **内置/本地供应商**、**外部供应商插件**、**扩展** 三组都列出来(扩展带开关,
走核心的 `PATCH /router/api/ext`)。子插件仍然是**各自独立安装的 npm 包**,
这一节只是让它们在父插件页里也看得见,不改变独立安装/卸载这件事本身 ——
官方那个「包含的组件」只认本 bundle 自己 patch 里声明的行,跨 bundle 没有注入通道。
