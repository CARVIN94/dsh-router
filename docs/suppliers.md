...
| `rate_limit` | 限流(429 类) |
| `quota` | 额度/权益不足 |
| `session_dead` | 凭证失效/凭证被上游拒(403 类,含风控误伤) |
| `unavailable` | 上游不可用(404 / 服务下线) |
| `transport` | 网络/连接层失败(没拿到 HTTP 状态) |
| `unknown` | 说不清是什么错 |
| `no_such_model` | **这个模型不属于本供应商**(不是账号的失败) |
| `bad_request` | **请求本身不合法,被上游拒**(参数错/图片认不出/tool_call 配对断裂;不是账号的失败) |
...
**`bad_request` 同理,而且更容易踩。** 同一个非法请求对池里**每个号**结果都相同
——冷号只会把「这条请求有问题」放大成「这个模型 30 秒内谁都别用」。2026-09-15
读图事故就是这么来的:适配器把带图的 tool-result 排错序导致 tool_call 配对断裂,
上游回 400/11148,插件当时归 `rate_limit` → money 组合两条腿同时被冷 30s →
**之后连纯文本请求**也全灭 503。判据:上游明确说「请求参数/形态不对」(如
CodeBuddy 的 `extError.type = invalid_request_error`,涵盖 11133/11135/11148/
11128)就报 `bad_request`;只是「上游忙/临时不可用」(11134)仍归 `rate_limit`,
那种情况换号确实管用。

**js 只说「现在怎么了」,不说「该怎么办」。** 冷却多久、要不要换号
都是核心的策略(见下),js 不该自己记冷却表——那样每个插件都会长出一份
互相不一致的实现。

核心在 `status()` 之上叠加 `cooling` / `err_count` / `until` / `reason`
...
例外是 `session_dead`:凭证失效是连接级问题,该号**所有模型**都停用(连接级冷却,到期自愈)。
...
| `quota` | 冷却 10 分钟 |
| `session_dead` | **连接级冷却 30 分钟**(该号所有模型都停用,到期自愈;没有永久禁用——403 分不清凭证死活与风控误伤) |
| `unavailable` / `transport` / `unknown` | 瞬时短冷却 30s(**每次都冷**,不攒次数) |
| `no_such_model` | **不惩罚账号**,核心直接跳过整个供应商换下一个 |
| `bad_request` | **不惩罚账号**(请求非法不是号的错),核心直接跳到下一个模型/供应商 |
...
冷却/退避等级都是**内存态**,重启归零(与凭证、积分的持久化不同)。
重启后所有号都会重新被选中一次,所以**上游确实挂了的号会各撞一次**,属预期。
要跨重启保留,届时给账号池加一个落盘 state 文件即可,接口不用变。

### 首字节预算:body 停摆由核心兜,js 不要自己设

上游常有一种「连上了、200 响应头也回来了、body 却一个字节都不吐」的停摆
(额度用完 / 过载时的静默排队)。`chatOnce` 已经把流交出去之后,js 就没有任何
「换号/换模型」的能力了,所以这个截止只能由**核心**设:

- 每条降级腿进核心起 60s(`FIRST_BYTE_BUDGET_MS`),上游还没吐第一个字节就判
  这条腿失败 → 按 `transport` 走换号/换模型(2026-09-24 现场:ttfb 84~255s
  的 8 次请求全挂在一条腿上,用户干等 200 多秒手动暂停)
- **第一个字节之后不再计时**,长生成不受影响
- 同一供应商的多个号共享这份预算(停滞是上游级的,按号各等一份会 60s×号数)

因此 js 侧的 `chatOnce` 超时**只守「连接 + 响应头」**就够了,不必、也不该给
body 再套一层空闲超时:响应头一到就把控制权交回核心,body 的事归核心。
...
两个来源，同 id 时**用户目录的 js 压过内置行**（`supplierWins`：user > builtin > external）：

1. **cordis service `router.suppliers`**（值为 `{ [supplierId]: (env) => SupplierModule }`），
   dsh-router `ctx.inject(['router.suppliers'])` 延迟加载（service 可用即注册）。两条子路径都走它：
   - **内置三个 = dsh-router-core 自己 patch 里的行**：`cordis.patch.yml` insert
     `dsh-router-core/suppliers/<x>` 三个子路径模块，每行一个 cordis 壳（`src/suppliers/<x>/index.ts`）
