...
export interface SupplierEnv {
   ...
   * 流内 `code=4008`，三个号里两个如此，选号一旦落到坏号就 2/3 请求直接失败
   * （2026-09-08 实测）——有了这条通道，坏号被冷一次就退出选号。
   *
...
export type AccountState =
  ...
  | 'no_such_model'
  /**
   * 请求本身不合法，被上游拒（参数错、图片格式不支持、tool_call 配对断裂…）。
   *
   * **不是账号的错**：同一个请求对池里每个号都会失败，冷号只会让好号白白
   * 失效，把一次「这条请求有问题」放大成「这个模型 30 秒内谁都别用」——
   * 组合里两条腿同时被冷就是这个机制（2026-09-15 读图 11148 事故）。
   * 核心据此**不惩罚账号**，直接跳出本供应商换下一个模型/供应商。
   * 与 `no_such_model` 的区别：那个是「模型不归我」，这个是「请求我不认」。
   */
  | 'bad_request'
