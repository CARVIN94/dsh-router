 ...
 *
 * 上游：https://opencode.ai/zen/v1（OpenAI 兼容，免费档）
 *   - chat:  POST /zen/v1/chat/completions（**只能流式**）
 *   - models: GET  /zen/v1/models（透传，只取免费模型）
 *
 * 「免费」不等于「无鉴权」：Zen 免费档仍要求**模拟真 CLI 的握手**，否则一律
 * 403 `FreeTierError: OpenCode's free tier can only be used from within OpenCode`
 * （2026-09-22 实测）。三个条件缺一即 403，见下方 `UA` / `sessionId` / `decoyTools`。
 * 免费档也没有账号可填 —— 所以无凭证/连接池/签到，面板只显示模型卡片
 * （连接池、签到由能力检测自动隐藏）。
 */
...
/**
 * User-Agent 必须带**版本号**且 ≥ 1.18。
 *
 * 裸 `opencode` → 403 FreeTierError；`opencode/1.17.x` → **426 UpgradeRequired**
 * （上游明确要求升级客户端）；`opencode/1.18.0` 起通过。别去掉版本号，
 * 也别写死成能过的最高版本——取一个已实测通过的稳定值即可。
 */
const UA = 'opencode/1.18.31'

/**
 * 免费档按 session 记账（每次生成一个新的即可——见 `chatOnce` 的实测说明），
 * `x-opencode-session` / `x-opencode-request` **必须**是上游认的形状，格式不对
 * 同样 403（不是忽略，是拒绝）：
 *   ses_<12 位小写 hex><14 位 base62>、msg_<同形状>
 */
function newSessionId(prefix: 'ses_' | 'msg_'): string {
  const B62 = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'
  const bytes = randomBytes(20)
  let time = ''
  for (let i = 0; i < 6; i++) time += bytes[i]!.toString(16).padStart(2, '0')
  let rand = ''
  for (let i = 6; i < 20; i++) rand += B62[bytes[i]! % 62] ?? ''
  return `${prefix}${time}${rand}`
}

/**
 * 每次请求一对新的 session/request id。
 *
 * 不做会话池的实测依据（2026-09-22）：每次新 session 与固定 session 各打 40 次
 * **全部 200**，且带大前缀时上游缓存命中数完全相同（都 2560 cached）——
 * 9router 的 `stableSessionId` 在这里没有可测收益，属未经验证的复杂度。
 */
function sessionHeaders(): Record<string, string> {
  return {
    'x-opencode-session': newSessionId('ses_'),
    'x-opencode-request': newSessionId('msg_'),
  }
}

/**
 * 免费档要求 tools 里**同时**存在 `bash` 和 `read`，否则 403。
 *
 * 为什么是「注入」而不是「覆盖」：上游只是**校验这两个名字在不在**，真正的
 * 工具照常工作（实测真实工具在前、占位在后，混合下发 200）。所以调用方已经
 * 带了同名工具时保持原样不动，缺哪个补哪个——绝不覆盖调用方自己的定义，
 * 否则会把真实工具签名糊掉。
 */
const DECOY_TOOLS: ReadonlyArray<{ name: string; description: string }> = [
  { name: 'bash', description: 'This tool is currently unavailable and must not be used.' },
  { name: 'read', description: 'This tool is currently unavailable and must not be used.' },
]
...
/**
 * 补上上游免费档要求的占位工具（幂等：已存在同名工具就跳过）。
 *
 * 只认 `function.name`（OpenAI chat 形态）——Zen 免费档是 chat/completions，
 * 不走 responses 的扁平 `name` 形态。
 */
function ensureDecoyTools(obj: Record<string, unknown>): void {
  const tools = Array.isArray(obj.tools) ? (obj.tools as Array<Record<string, unknown>>) : []
  const names = new Set<string>()
  for (const t of tools) {
    const fn = t?.function as Record<string, unknown> | undefined
    const n = typeof fn?.name === 'string' ? fn.name : typeof t?.name === 'string' ? t.name : ''
    if (n !== '') names.add(n)
  }
  for (const d of DECOY_TOOLS) {
    if (names.has(d.name)) continue
    tools.push({
      type: 'function',
      function: { name: d.name, description: d.description, parameters: { type: 'object', properties: {} } },
    })
  }
  obj.tools = tools
}
...
export default function factory(env: SupplierEnv): SupplierModule {
  ...
  async function refreshModels(): Promise<ModelInfo[]> {
    try {
      // /models 无握手要求（实测裸 curl 也 200），不必带 session 头
  ...
  return {
    ...
    async chatOnce(_uid: string, lv: string, req: ChatRequest): Promise<ChatOnceResult> {
      ...
      // 模型名规范化 + 握手补齐。解析失败也要继续发（宁可不改，不能不发）：
      // 上游只认 JSON，坏 body 由它按 400 拒，那正是我们要如实上报的状态。
      let body = req.rawBody
      try {
        const obj = JSON.parse(body) as Record<string, unknown>
        obj.model = stripAlias(String(obj.model ?? req.model), currentAlias())
        // 免费档**只支持流式**：`stream:false` 即使握手齐全也 403（实测）。
        // 客户端要非流式时代理层会聚合 SSE，所以这里恒为 true。
        obj.stream = true
        // 上游要求 tools 里同时有 bash/read，否则 403（缺哪个补哪个）
        ensureDecoyTools(obj)
        // effort 白名单：'off'/'none' 必须**删字段**（上游没有这两个值，
        // 传 'off' 直接 400 invalid_request_error）；low/high/max 实测可过。
        // 'auto' 保持调用方原样不动（可能带了自己的 reasoning_effort）。
      ...
      try {
        upstream = await fetch(CHAT_URL, {
          ...
          headers: {
            ...
            // 免费档固定字面量 'public'；实测缺这行也 200，但保留以免上游收紧
      ...
      // 上游只有流式（上面已强制 stream:true），所以这里恒回流。
      // 客户端要非流式时由核心的 aggregateSSE 聚合——协议错配归核心吸收。
      if (!upstream.body) {
