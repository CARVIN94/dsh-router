...
/**
 * 「添加 API key」弹窗提示（OpenRouter API key 获取页）。
 */
export const apiKeyHint = '填入 OpenRouter API key（https://openrouter.ai/settings/keys），可起名字区分多个 key。'
  ...
  return {
    ...
    async chatOnce(uid: string, lv: string, req: ChatRequest): Promise<ChatOnceResult> {
      ...
      if (upstream.status < 200 || upstream.status >= 300) {
        ...
        // 401/403 = 这个 key 本身不可用；429 = 限流；其余见 stateFromHttpStatus
